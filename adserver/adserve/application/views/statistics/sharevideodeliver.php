					...
					<div class="box-body">
						...
						<table id="example" style="margin-top:15px;" class="table table-striped" >
							...
							<tbody>
								...
								$bannerCount	= count($vastEventStartData);
								...
								for($k=0;$k<=$totolDays;$k++){ $reportDate = date('Y-m-d', strtotime($reportDate .' -1 day'));  if($reportDate >  '2018-05-16' ){ continue;}
									...
									for($i=0;$i<=$bannerCount-1;$i++){
										foreach($vastEventStartData[$i] as $key=>$value){
										...
										foreach($vastEventFirstData[$i] as $key1=>$value1){ 
											 if($reportDate == $value1->day){
										...
										foreach($vastEventMidData[$i] as $key2=>$value2){ //echo '<pre>';print_r($value2);die;
											if($reportDate == $value2->day){ 
										...
										foreach($vastEventThirdData[$i] as $key3=>$value3){
											if($reportDate == $value3->day){ //echo '<pre>';print_r($value);die;
												$vastEventDescription['thirdQuad'] += $value3->total;
												break;
											}
										}
										
										foreach($vastEventCompleteData[$i] as $key4=>$value4){
											if($reportDate == $value4->day){ //echo '<pre>';print_r($value);die;
										...
										//echo '<pre>';print_r($vastEventDescription);die;
									?>
									...
									<td style="text-align:center;">
										<?php  echo $vastEventDescription['start']*$n;?>
									</td>
									<td style="text-align:center;">
										<?php  echo $vastEventDescription['firstQuad']*$n;?>
									</td>
									<td style="text-align:center;">
										<?php  echo $vastEventDescription['midPoint']*$n;?>
									
									</td>
									<td style="text-align:center;">
										<?php  echo $vastEventDescription['thirdQuad']*$n;?>
									
									</td>
									
									<td style="text-align:center;">
										<?php  echo $vastEventDescription['complete']*$n;?>
									</td>
								</tr> 
