...
<div class="content-wrapper">
    <section class="content-header">
		...
		<h1 style="padding-left: 10px;margin-top:12px;"><?php echo $zoneData[0]->name;?></h1>
    </section>
				...
				<?php $this->load->view("admin_includes/zoneheader");?>
					<form method="post" name="addbanner" id="addbanner">
						...
						<?php if(isset($this->session->userdata['zonedata'])){ ?>
							...
							<div class="errormessage" id="warning_change_zone_size" style="display:<?php if($msg['msgType']=='warning'){ echo 'block'; }else{ echo 'none';} ?>;"> 
							<img class="errormessage" src="<?php echo base_url();?>assets/upimages/warning.gif" align="absmiddle">
								<span class="tab-s"> Notice:</span><?php echo $msg['msg']; ?>
								
							</div>
							<div class="message localMessage" style="display:<?php if($msg['msgType']=='success'){ echo 'block'; }else{ echo 'none';} ?>; ">
						...
						<?php }?>
							...
							<div class="col-md-7">
								<div class="form-group" style="">
									...
									<input type="hidden" name="affiliateid" value="<?php if(isset($_GET['affiliateid'])){echo $_GET['affiliateid'];} ?>">
									<input type="hidden" name="zoneid" value="<?php if(isset($_GET['zoneid'])){ echo $_GET['zoneid'];}?>">
										...
										<option value="<?php echo $value->clientid;?>" <?php if (isset($_GET['clientid']) && $_GET['clientid'] == $value->clientid) echo 'selected="selected"'; ?>><?php echo $value->clientname;?> </option>
										<?php }} ?>
									</select>
								</div>
								<div class="form-group" style="margin-top: 20px;display:<?php if(!(isset($_GET['clientid']))){ ?> none;<?php }else{?> block<?php } ?>">
								...
								<form name='zonetypeselection' method='get' action=''>
									...
									<input type="hidden" name="clientid" value="<?php if(isset($_GET['clientid'])){ echo $_GET['clientid'];}?>">
										...
										<option value="<?php echo $value->campaignid;?>" <?php if (isset($_GET['campaignid']) && $_GET['campaignid'] == $value->campaignid) echo 'selected="selected"'; ?>><?php echo $value->campaignname;?> </option>
										<?php } ?>
									</select>
									<!--
								...
								<div class="form-group" style="margin-top: 20px;display:<?php if(!(isset($_GET['campaignid']))){ ?> none;<?php }else{?> block<?php } ?>">
								<form name='zonetypeselection' method='get' action=''>
									<input type="hidden" name="affiliateid" value="<?php if(isset($_GET['affiliateid'])){echo $_GET['affiliateid'];} ?>">
									<input type="hidden" name="zoneid" value="<?php if(isset($_GET['zoneid'])){ echo $_GET['zoneid'];}?>">
									<input type="hidden" name="clientid" value="<?php if(isset($_GET['clientid'])){ echo $_GET['clientid'];}?>">
									<input type="hidden" name="campaignid" value="<?php if(isset($_GET['campaignid'])){ echo $_GET['campaignid'];}?>">
										...
										<img align="absmiddle" src="<?php echo base_url()?>/assets/upimages/icon-banner-stored.gif">
										...
										<option value="<?php echo $value->bannerid;?>"><?php echo $value->description;?></option>
										<?php } ?>
										</select>
									<input id="link_submit" name="submitimage" type="image" src="<?php echo base_url();?>assets/upimages/ltr/go_blue.gif" border="0" tabindex="2">									
								</form>
							...
							<table id="example" class="table table-bordered table-striped">
								...
								<tbody>
									...
									<tr>
										<td><img src="<?php echo base_url();?>/assets/upimages/<?php if($value->storagetype=='html'){echo 'icon-banner-html.png';}else{?>icon-banner.png<?php } ?>">
										<a href="<?php echo base_url();?>users/banner?bannerid=<?php echo $value->bannerid;?>&campaignid=<?php echo $value->campaignid;?>&clientid=<?php echo $value->clientid;?>"><?php echo $value->description;?></a></td>
										<td><?php echo $value->bannerid;?></td>
										<td><a href="<?php echo base_url();?>users/invocation?bannerid=<?php echo $value->bannerid;?>&campaignid=<?php echo $value->campaignid;?>&clientid=<?php echo $value->clientid;?>"><div class="btn bg-green btn-xs">Invocation Code</div></a></td>
									</tr>                 
									<?php } ?>
								</tbody>
									<?php }else{ ?>
...
$("#clientid").change(function(){
		...
		window.location.href		= siteurl+"users/zone_include?affiliateid=<?php echo $_GET['affiliateid'];?>&zoneid=<?php echo $_GET['zoneid'];?>&clientid="+clientid;
	});
</script>
<script>
$("#campaignid").change(function(){
	var campaignid			= $(this).val();
	window.location.href	= siteurl+"users/zone_include?affiliateid=<?php echo $_GET['affiliateid'];?>&zoneid=<?php echo $_GET['zoneid'];?>&clientid=<?php if(isset($_GET['clientid'])){echo $_GET['clientid'];}?>&campaignid="+campaignid;
