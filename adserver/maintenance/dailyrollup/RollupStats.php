 ...
 */
		...
		/* mysqli_query($oDbh,"DROP TABLE IF EXISTS {$this->prefix}tmp_tz_offset");
		mysqli_query($oDbh,"CREATE TABLE `{$this->prefix}tmp_tz_offset`
                               (`timezone` varchar(32) NOT NULL PRIMARY KEY, `offset` char(6) NOT NULL DEFAULT '+00:00') ENGINE=innoDb");
       
        mysqli_query($oDbh,"INSERT INTO {$this->prefix}tmp_tz_offset (timezone, offset) VALUES('Asia/Calcutta', '+05:30')");
      
      
        // CREATE an ad_id => offset table
        mysqli_query($oDbh,"DROP TABLE IF EXISTS {$this->prefix}tmp_ad_offset");
        mysqli_query($oDbh,"CREATE TABLE `{$this->prefix}tmp_ad_offset`
                               (`ad_id` int(11) NOT NULL PRIMARY KEY, `offset` char(6) NOT NULL DEFAULT '+00:00') ENGINE=innoDb");

        mysqli_query($oDbh,"INSERT INTO {$this->prefix}tmp_ad_offset SELECT bannerid,'+05:30' FROM banners AS b");
     
	   mysqli_query($oDbh,"UPDATE
        ...
        // mysqli_query($oDbh,"DROP TABLE IF EXISTS {$this->prefix}data_summary_ad_hourly_backup");
 ...
 */
        ...
        //mysqli_query($oDbh,"RENAME TABLE {$this->prefix}data_summary_ad_hourly TO {$this->prefix}data_summary_ad_hourly_backup");
       
		//mysqli_query($oDbh,"RENAME TABLE {$this->prefix}data_summary_ad_hourly_rolledup TO {$this->prefix}data_summary_ad_hourly");
		
        // Cleanup
         //mysqli_query($oDbh,"DROP TABLE {$this->prefix}tmp_ad_offset;");
         //mysqli_query($oDbh,"DROP TABLE {$this->prefix}tmp_tz_offset;");
    }
